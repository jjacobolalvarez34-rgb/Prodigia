...
-- K-factor de ELO escalado por rango en vez de un valor fijo (13
-- para todos). Bronce ~20 (los cambios de rating pegan fuerte
-- cuando recién estás calibrando), Diamante ~10 (más estable
-- arriba), Plata/Oro/Platino interpolados linealmente entre los
-- dos. Prodigio (el rango arriba de Diamante, ver RANGOS_ELO en
-- types/database.ts) se queda en el mismo piso que Diamante — no
-- pedido explícitamente, pero bajarlo más no tendría base.
--
-- Cada jugador usa SU PROPIO K (según su propio ELO, no el
-- promedio de los dos) — mismo criterio que sistemas de rating
-- tipo FIDE/USCF, donde el K depende de la experiencia/rating de
-- cada jugador individualmente, no del par.
-- ============================================================

create or replace function public.k_factor_de_elo(p_elo integer)
returns numeric
language sql
immutable
as $$
  select case
    when p_elo >= 1500 then 10
    when p_elo >= 1300 then 12.5
    when p_elo >= 1100 then 15
    when p_elo >= 900 then 17.5
    else 20
  end;
$$;

grant execute on function public.k_factor_de_elo(integer) to authenticated;
...
begin
  ...
  v_nuevo_elo := round(v_mi_elo + v_k_mi * (v_actual - v_esperado));
  ...
  update public.profiles
    set elo_rating = round(v_otro_elo + v_k_otro * ((1 - v_actual) - (1 - v_esperado)))
    where id = v_otro_id;
