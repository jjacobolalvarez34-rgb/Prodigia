...
-- finalizar_el_reloj, apostar_casino_elementos. Las que coinciden en
-- el texto "set puntos_total = puntos_total" pero NO tienen el bug
-- (porque su `returns` no declara una columna "puntos_total" que
-- choque) se dejan intactas a propósito: resolver_apuesta_partida y
...
begin
  ...
  update public.profiles as pr set puntos_total = pr.puntos_total - 100 where pr.id = v_user;
...
begin
  ...
  update public.profiles as pr set puntos_total = pr.puntos_total - 60 where pr.id = v_user;
