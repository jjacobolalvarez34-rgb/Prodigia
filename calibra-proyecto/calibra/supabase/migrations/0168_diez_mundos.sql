-- ============================================================
-- Prodigia — Cierre cross-cutting del mundo 10 (Circuitia): las
-- funciones que enumeran los mundos a mano (elegir_mundos_iniciales,
-- desbloquear_mundo, comprar_item_tienda, el paquete de marcos) pasan
-- de 9 a 10.
-- Correr después de 0167_mundo_circuitia.sql.
--
-- ⚠️ REGLA de docs/PARIDAD_MUNDOS.md, seguida acá al pie de la letra:
-- desbloquear_mundo se copia del CUERPO de 0166_nueve_mundos.sql (la
-- versión ya arreglada y vigente, ella misma copiada de 0111 en su
-- momento), nunca de una migración más vieja — ese mismo bug de columna
-- ambigua (42702, `select puntos_total, mundos_desbloqueados into ...`
-- sin alias, que choca contra las variables de salida homónimas de
-- `returns table`) se rompió DOS veces en la sesión que agregó
-- Trigonometría/Historia. Cada referencia a una columna de `profiles`
-- dentro de las funciones de abajo usa un alias (`pr.puntos_total`,
-- `pr.mundos_desbloqueados`), incluido el primer SELECT ... INTO.
--
-- elegir_mundo_inicial (singular) sigue revocada desde 0116 — el flujo
-- real de onboarding solo usa elegir_mundos_iniciales (plural, elige 2
-- mundos gratis a la vez), así que es ESA la que hace falta extender
-- acá, con el mismo cuidado de alias.
-- ============================================================

-- ---------- 1) elegir_mundos_iniciales (base real: 0166): + circuitia ----------
...
declare
  ...
  v_actuales text[];
  v_validos constant text[] := array['numeria', 'geografia', 'enigmia', 'quimia', 'anatomia', 'melodia', 'trigonometria', 'historia', 'calculia', 'circuitia'];
  v_mundo text;
begin
...
-- ---------- 2) desbloquear_mundo: copia exacta del cuerpo de 0166 (versión ya arreglada) + circuitia ----------
...
-- ---------- 3) profiles.marco_perfil: + circuitia ----------
...
alter table public.profiles add constraint profiles_marco_perfil_check
  check (marco_perfil in (
    ...
    'numeria', 'enigmia', 'geografia', 'quimia', 'anatomia', 'melodia', 'trigonometria', 'historia', 'calculia', 'circuitia'
  ));

-- ---------- 4) comprar_item_tienda: copia exacta de la versión vigente (0166) + marco_circuitia + paquete a 10 mundos ----------
...
declare
  ...
  v_plan text;
  v_categoria text;
  v_mundos constant text[] := array['numeria', 'enigmia', 'geografia', 'quimia', 'anatomia', 'melodia', 'trigonometria', 'historia', 'calculia', 'circuitia'];
...
begin
  ...
  v_costo_base := case p_item
    ...
    when 'paquete_marcos_mundo' then 18000
  ...
  if p_item in ('marco_numeria', 'marco_enigmia', 'marco_geografia', 'marco_quimia', 'marco_anatomia', 'marco_melodia', 'marco_trigonometria', 'marco_historia', 'marco_calculia', 'marco_circuitia') then
    ...
    if exists (
      ...
      raise exception 'todavia no alcanzaste nivel 40 en los 10 mundos';
    end if;
  end if;
...
-- Mismo criterio que 0166_nueve_mundos.sql — otorgar_marco_mundo(uuid,
-- text) ya es genérico (no hardcodea el mundo), solo hace falta el
-- loop nuevo con el criterio de Circuitia (4/4 sub-tipos en nivel >=
-- 10), mismo umbral que mundoCompletado en src/lib/titulos/verificar.ts.
do $$
declare
  v_user record;
  v_completo boolean;
begin
  for v_user in select id from public.profiles loop
    ...
    where user_id = v_user.id and problem_type in ('circuitia_serie', 'circuitia_paralelo', 'circuitia_mixto', 'circuitia_cualitativo');
    if v_completo then perform public.otorgar_marco_mundo(v_user.id, 'circuitia'); end if;
  end loop;
end $$;
