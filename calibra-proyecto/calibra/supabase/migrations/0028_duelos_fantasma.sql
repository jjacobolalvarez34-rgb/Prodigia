-- ============================================================
-- Prodigia — fantasma del rival en duelos (cierre de Fase J2)
-- Correr después de 0027_geografia_lecciones.sql
-- Los duelos siguen siendo asincrónicos (no hay sesión en vivo
-- compartida entre dos personas conectadas al mismo tiempo — eso queda
-- anotado como proyecto aparte, no se encara acá). Lo que se agrega es
-- más simple: si tu rival ya jugó su lado del duelo antes que vos,
-- guardamos su secuencia de respuestas (acierto + tiempo por problema)
-- en duel_results.respuestas, y cuando jugás el tuyo se te muestra un
-- "fantasma" que avanza al ritmo exacto de esas respuestas ya
-- guardadas, en paralelo a tu partida en vivo.
-- ============================================================

alter table public.duel_results add column respuestas jsonb;

drop function if exists public.registrar_resultado_duelo(uuid, numeric, numeric, integer);
