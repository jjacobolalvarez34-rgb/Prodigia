-- ============================================================
-- Prodigia — Sección 6: rebalanceo de economía + piso de precio real
-- Correr después de 0060_auditoria_rls_2.sql
--
-- comprar_item_tienda(p_item, p_costo) confía en el p_costo que manda
-- el cliente — api/tienda/comprar/route.ts SIEMPRE lo calcula
-- server-side (nunca confía en lo que mande el navegador para el
-- precio final), así que el flujo real de la app está bien. Pero la
-- función en sí queda callable directo (grant execute a authenticated,
-- necesario para que la API route funcione) con cualquier p_costo
-- desde la consola del navegador — con sesión real, nada le impide
-- pasar p_costo=1 y comprar el marco Prodigio por 1 Chispa. Esto
-- vuelve inútil el rebalanceo de precios de esta misma sección si
-- alguien lo explota. Fix: un piso de precio real por ítem dentro de
-- la función — no duplica la lógica de descuento diario completa (el
-- hash determinístico por fecha vive en TS, portarla a SQL no vale la
-- pena para esto), pero el descuento máximo del día es 50% (ver
-- src/lib/descuentoDiario.ts, porcentajes = [20,30,40,50]), así que
-- ningún precio legítimo baja de la mitad del precio de catálogo.
-- ============================================================
...
begin
  ...
  -- Piso de precio real (Sección 6) — el catálogo vigente vive en
  -- src/lib/tienda/costos.ts; esto es el mismo catálogo en SQL, solo
  -- para validar que nadie pague menos de la mitad (el descuento
  -- diario máximo posible).
