...
import type { ModoTrigonometria } from "@/lib/practica/trigonometria";

export interface DueloTrigonometriaInfo {
  duelId: string;
  rivalId: string;
  rivalNombre: string;
  miElo: number;
...
export interface DatosPracticaTrigonometria {
  modo: ModoTrigonometria;
  nivelInicial: number;
  escudosExtra: number;
  boostActivo: boolean;
  dueloInfo: DueloTrigonometriaInfo | null;
}

const MODOS_VALIDOS: ModoTrigonometria[] = ["razones", "circulo", "identidades", "leyes"];

// Mismo patrón que cargarDatosPracticaMelodia — sin semilla, cada rival
// resuelve su propio contenido al azar a la misma dificultad.
export async function cargarDatosPracticaTrigonometria(
  supabase: SupabaseClient,
  userId: string,
  modoDeLaRuta: ModoTrigonometria,
  duelo?: string
): Promise<DatosPracticaTrigonometria> {
  let dueloInfo: DueloTrigonometriaInfo | null = null;
  ...
  const [{ data: nivelRow }, { data: profile }] = await Promise.all([
    supabase.from("skill_levels").select("nivel").eq("user_id", userId).eq("problem_type", `trigonometria_${modo}`).maybeSingle(),
