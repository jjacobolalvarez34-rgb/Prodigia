...
export interface DueloCalculiaInfo {
  duelId: string;
  rivalId: string;
  rivalNombre: string;
  miElo: number;
...
export interface DatosPracticaCalculia {
  modo: ModoCalculia;
  nivelInicial: number;
  escudosExtra: number;
  ...
  boostActivo: boolean;
  dueloInfo: DueloCalculiaInfo | null;
}

const MODOS_VALIDOS: ModoCalculia[] = ["derivadas", "integrales", "series", "multivariable"];
...
export async function cargarDatosPracticaCalculia(
  supabase: SupabaseClient,
  userId: string,
  modoDeLaRuta: ModoCalculia,
  duelo?: string
): Promise<DatosPracticaCalculia> {
  let dueloInfo: DueloCalculiaInfo | null = null;
  ...
  const [{ data: nivelRow }, { data: profile }] = await Promise.all([
    supabase.from("skill_levels").select("nivel").eq("user_id", userId).eq("problem_type", `calculia_${modo}`).maybeSingle(),
