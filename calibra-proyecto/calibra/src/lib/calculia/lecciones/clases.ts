// Las 7 Clases (Pro) de Calculia ("Curso estructurado", 0170), con `pasos` y
// `quiz` tal como quedaron tras 0195 (notación LaTeX). lecciones.test.ts lo
// compara contra las migraciones sembradas. Lo único nuevo son los `visuales`
// (migración 0216, generada por sql.ts). El orden 6..12 es el ya sembrado; el
// desbloqueo es POR TEMA (src/lib/calculia/path.ts).

import type { LeccionCalculia } from "./tipos";

export const CLASES_CALCULIA: LeccionCalculia[] = [
  {
    slug: "calculia-pro-derivadas-fundamentos",
    grupo: "derivadas",
    orden: 6,
    requierePro: true,
    nombre: "Curso Pro: Derivadas desde cero — la regla de la potencia",
    descripcion: "Qué significa derivar y cómo aplicar la regla de la potencia paso a paso, con un ejemplo resuelto completo.",
    pasos: [
    ...
    quiz: [
      {
        pregunta: "Deriva $f(x) = 6x^{3}$ respecto de x.",
        opciones: ["$18x^{2}$", "$6x^{2}$", "$18x^{3}$", "$3x^{2}$"],
        respuesta: "$18x^{2}$",
        explicacion: "Bajas el exponente (3) como factor: $6\\cdot 3=18$, y le restas 1 al exponente: $x^{3} \\to x^{2}$. Da $18x^{2}$.",
      },
      {
        pregunta: "¿Cuál es la derivada de una función constante, como $f(x) = 9$?",
        opciones: ["0", "9", "1", "$9x$"],
        respuesta: "0",
        explicacion: "Una constante no cambia nunca — su pendiente (razón de cambio) es siempre 0, sin importar el valor de la constante.",
      },
      {
        pregunta: "Deriva $f(x) = 2x^{5}$ respecto de x.",
        opciones: ["$10x^{4}$", "$2x^{4}$", "$10x^{5}$", "$5x^{4}$"],
        respuesta: "$10x^{4}$",
        explicacion: "Coeficiente 2 por exponente $5 = 10$, y el exponente baja de 5 a 4: $10x^{4}$.",
      },
    ],
    visuales: [
      {
  ...
  {
    slug: "calculia-pro-derivadas-producto-cociente-cadena",
    grupo: "derivadas",
    orden: 7,
    requierePro: true,
    nombre: "Curso Pro: Derivadas — producto, cociente y cadena",
    descripcion: "Las tres reglas que hacen falta cuando la potencia sola no alcanza: multiplicación de expresiones, división de expresiones, y funciones \"adentro de otra función\".",
    pasos: [
    ...
    quiz: [
      {
        pregunta: "Deriva $f(x) = (2x+1)^{3}$ usando la regla de la cadena.",
        opciones: ["$6(2x+1)^{2}$", "$3(2x+1)^{2}$", "$6(2x+1)^{3}$", "$2(2x+1)^{2}$"],
        respuesta: "$6(2x+1)^{2}$",
        explicacion: "La de afuera da $3(2x+1)^{2}$, y hay que multiplicar por la derivada de adentro $(2x+1)' = 2$: $3\\cdot 2=6$, queda $6(2x+1)^{2}$.",
      },
      {
        pregunta: "¿Qué regla usas para derivar $f(x) = \\frac{x^{2}+1}{x+3}$?",
        opciones: ["Regla del cociente", "Regla del producto", "Regla de la cadena", "Regla de la potencia sola"],
        respuesta: "Regla del cociente",
        explicacion: "Es una DIVISIÓN de dos expresiones — eso siempre pide la regla del cociente, con el orden $\\text{alto}'\\cdot\\text{bajo}$ menos $\\text{alto}\\cdot\\text{bajo}'$ en el numerador.",
      },
      {
        pregunta: "Al aplicar la regla del producto a $f(x) = u(x)v(x)$, ¿cuál es la fórmula correcta?",
        opciones: ["$u'v + uv'$", "$u'v'$", "$u'v - uv'$", "uv"],
        respuesta: "$u'v + uv'$",
        explicacion: "Es una SUMA de dos términos — derivada del primero por el segundo sin derivar, más el primero sin derivar por la derivada del segundo. Multiplicar $u'\\cdot v'$ directamente es el error más común.",
      },
    ],
    visuales: [
      {
  ...
  {
    slug: "calculia-pro-integrales-fundamentos",
    grupo: "integrales",
    orden: 8,
    requierePro: true,
    nombre: "Curso Pro: Integrales desde cero — la antiderivada",
    descripcion: "Qué es integrar (deshacer una derivada) y cómo aplicar la regla de la potencia al revés, con ejemplo resuelto y verificación.",
    pasos: [
    ...
    quiz: [
      {
        pregunta: "Calcula $\\int 12x^{3}\\,dx$.",
        opciones: ["$3x^{4} + C$", "$12x^{4} + C$", "$3x^{3} + C$", "$4x^{4} + C$"],
        respuesta: "$3x^{4} + C$",
        explicacion: "El exponente sube de 3 a 4, y el coeficiente 12 se divide por ese exponente nuevo (4): $\\frac{12}{4}=3$. Queda $3x^{4} + C$.",
      },
      {
        pregunta: "¿Por qué toda integral indefinida lleva \"+ C\"?",
        opciones: ["Porque cualquier constante desaparece al derivar, así que hay infinitas antiderivadas posibles", "Es una convención sin motivo matemático", "Porque C siempre vale 0", "Solo se agrega en integrales de potencias"],
        respuesta: "Porque cualquier constante desaparece al derivar, así que hay infinitas antiderivadas posibles",
        explicacion: "Si F(x) es una antiderivada de $f(x)$, F(x)+C también lo es para cualquier constante C, porque la derivada de una constante es 0.",
      },
      {
        pregunta: "Calcula $\\int (\\frac{1}{x})\\,dx$.",
        opciones: ["$\\ln|x| + C$", "$\\frac{1}{x^{2}} + C$", "$x^{0} + C$", "$-\\frac{1}{x^{2}} + C$"],
        respuesta: "$\\ln|x| + C$",
        explicacion: "Es el único caso donde la regla de la potencia no funciona ($n=-1$ haría dividir por cero) — la antiderivada de $\\frac{1}{x}$ es siempre el logaritmo natural.",
      },
    ],
    visuales: [
      {
  ...
  {
    slug: "calculia-pro-integrales-avanzadas",
    grupo: "integrales",
    orden: 9,
    requierePro: true,
    nombre: "Curso Pro: Integrales — exponencial, trigonométricas y sustitución",
    descripcion: "Tres formas más allá de la potencia: la exponencial, seno/coseno, y cómo reconocer cuándo conviene sustitución simple.",
    pasos: [
    ...
    quiz: [
      {
        pregunta: "Calcula $\\int e^{3x}\\,dx$.",
        opciones: ["$\\frac{1}{3}e^{3x} + C$", "$e^{3x} + C$", "$3e^3x + C$", "$\\frac{1}{3}e^{x} + C$"],
        respuesta: "$\\frac{1}{3}e^{3x} + C$",
        explicacion: "La exponencial se mantiene igual, pero hay que dividir por el coeficiente del exponente (3) para que la derivada de la respuesta vuelva a dar $e^{3x}\\cdot 3$.",
      },
      {
        pregunta: "Calcula $\\int \\operatorname{sen}(x)\\,dx$.",
        opciones: ["$-\\cos(x) + C$", "$\\cos(x) + C$", "$-\\operatorname{sen}(x) + C$", "$\\operatorname{sen}(x) + C$"],
        respuesta: "$-\\cos(x) + C$",
        explicacion: "La derivada de $-\\cos(x)$ es $\\operatorname{sen}(x)$ (porque la derivada de $\\cos(x)$ es $-\\operatorname{sen}(x)$, y el signo se cancela) — coincide con el integrando original.",
      },
      {
        pregunta: "Calcula $\\int 10(2x+3)^{4}\\,dx$ usando sustitución $u=2x+3$.",
        opciones: ["$(2x+3)^{5} + C$", "$5(2x+3)^{5} + C$", "$10(2x+3)^{5} + C$", "$(2x+3)^{4} + C$"],
        respuesta: "$(2x+3)^{5} + C$",
        explicacion: "$a(n+1)=2\\cdot 5=10$, y el integrando ya trae ese coeficiente (10), así que en la respuesta queda $\\frac{10}{10}=1$: $(2x+3)^{5} + C$.",
      },
    ],
    visuales: [
      {
  ...
  {
    slug: "calculia-pro-series-geometricas",
    grupo: "series",
    orden: 10,
    requierePro: true,
    nombre: "Curso Pro: Series — geométricas, convergencia y suma infinita",
    descripcion: "Cuándo una serie geométrica converge, cómo calcular su suma infinita, y la diferencia con una serie p.",
    pasos: [
    ...
    quiz: [
      {
        pregunta: "Calcula la suma de la serie geométrica infinita con primer término $a=6$ y razón $r=\\frac{1}{3}$.",
        opciones: ["9", "18", "6", "2"],
        respuesta: "9",
        explicacion: "$S = \\frac{a}{1-r} = \\frac{6}{1-\\frac{1}{3}} = \\frac{6}{\\frac{2}{3}} = 6\\cdot\\frac{3}{2} = 9$. Primero se confirma que $\\left|\\frac{1}{3}\\right|<1$, así que converge.",
      },
      {
        pregunta: "¿La serie geométrica con razón $r = \\frac{3}{2}$ converge o diverge?",
        opciones: ["Diverge", "Converge", "Converge solo si $a>0$", "No se puede saber sin el primer término"],
        respuesta: "Diverge",
        explicacion: "$\\left|\\frac{3}{2}\\right| = 1.5$, que es mayor o igual a 1 — la condición de convergencia $|r|<1$ no se cumple, así que diverge.",
      },
      {
        pregunta: "¿Para qué valores de p converge la serie p, $\\sum \\frac{1}{n^{p}}$?",
        opciones: ["$p > 1$", "$p < 1$", "Cualquier p", "$p = 1$"],
        respuesta: "$p > 1$",
        explicacion: "El criterio de convergencia de una serie p es $p>1$ — un criterio totalmente distinto al de la razón $|r|<1$ de las series geométricas, no hay que mezclarlos.",
      },
    ],
    visuales: [
      {
  ...
  {
    slug: "calculia-pro-multivariable-parciales",
    grupo: "multivariable",
    orden: 11,
    requierePro: true,
    nombre: "Curso Pro: Multivariable — derivadas parciales desde cero",
    descripcion: "Cómo derivar una función de dos variables tratando la otra como si fuera una constante, con ejemplo resuelto completo.",
    pasos: [
    ...
    quiz: [
      {
        pregunta: "Para $f(x,y) = 5x^{3}y^{2}$, calcula $\\frac{\\partial f}{\\partial x}$.",
        opciones: ["$15x^{2}y^{2}$", "$5x^{2}y^{2}$", "$15x^{3}y$", "$10x^{3}y$"],
        respuesta: "$15x^{2}y^{2}$",
        explicacion: "Tratando $y^{2}$ como constante: $5\\cdot 3=15$, el exponente de x baja de 3 a 2, y la potencia de y $(y^{2})$ queda igual: $15x^{2}y^{2}$.",
      },
      {
        pregunta: "Para la misma $f(x,y) = 5x^{3}y^{2}$, calcula $\\frac{\\partial f}{\\partial y}$.",
        opciones: ["$10x^{3}y$", "$5x^{3}y$", "$15x^{2}y^{2}$", "$10x^{2}y$"],
        respuesta: "$10x^{3}y$",
        explicacion: "Tratando $x^{3}$ como constante: $5\\cdot 2=10$, el exponente de y baja de 2 a 1, y la potencia de x $(x^{3})$ queda igual: $10x^{3}y$.",
      },
      {
        pregunta: "Al calcular $\\frac{\\partial f}{\\partial x}$ de $f(x,y)=x^{2}y^{4}$, ¿qué error es más común?",
        opciones: ["Derivar también la y, en vez de tratarla como constante fija", "Olvidarse del signo negativo", "Multiplicar por y en vez de x", "Usar la regla del producto"],
        respuesta: "Derivar también la y, en vez de tratarla como constante fija",
        explicacion: "El error típico es derivar las DOS variables a la vez — cuando piden $\\frac{\\partial}{\\partial x}$, la potencia de $y$ del resultado tiene que quedar IGUAL que en el original, sin tocarla.",
      },
    ],
    visuales: [
      {
  ...
  {
    slug: "calculia-pro-edos-separables",
    grupo: "multivariable",
    orden: 12,
    requierePro: true,
    nombre: "Curso Pro: EDOs separables — separar variables e integrar",
    descripcion: "Cómo resolver una ecuación diferencial separable paso a paso, desde separar las variables hasta despejar y.",
    pasos: [
    ...
    quiz: [
      {
        pregunta: "¿Cuál es el primer paso para resolver una EDO separable $\\frac{dy}{dx} = f(x)\\cdot g(y)$?",
        opciones: ["Separar: $\\frac{dy}{g(y)} = f(x)\\,dx$", "Integrar directamente sin reordenar", "Derivar ambos lados", "Reemplazar y por una constante"],
        respuesta: "Separar: $\\frac{dy}{g(y)} = f(x)\\,dx$",
        explicacion: "El nombre \"separable\" viene de este paso — hay que dejar toda la y (con $dy$) de un lado y toda la x (con $dx$) del otro ANTES de integrar.",
      },
      {
        pregunta: "Resuelve la EDO separable $\\frac{dy}{dx} = 3x^{2}y$ (deja la respuesta en términos de la constante A).",
        opciones: ["$y = A\\cdot e^{x^{3}}$", "$y = A\\cdot e^{3x^{2}}$", "$y = A\\cdot x^{3}$", "$y = A\\cdot e^{x^{2}}$"],
        respuesta: "$y = A\\cdot e^{x^{3}}$",
        explicacion: "Separando: $\\frac{dy}{y}=3x^{2}\\,dx$. Integrando: $\\ln|y|=x^{3}+C_{1}$ (la antiderivada de $3x^{2}$ es $x^{3}$). Despejando $y$ con exponencial: $y=A\\cdot e^{x^{3}}$.",
      },
      {
        pregunta: "Al despejar y de $\\ln|y| = x^{2} + C_{1}$, ¿qué operación hay que aplicar en ambos lados?",
        opciones: ["La función exponencial (elevar e a cada lado)", "El logaritmo otra vez", "Elevar al cuadrado", "Dividir por $x^{2}$"],
        respuesta: "La función exponencial (elevar e a cada lado)",
        explicacion: "Exponencial y logaritmo natural son operaciones inversas — aplicar $e^{(\\cdots)}$ a ambos lados de $\\ln|y| = \\cdots$ deshace el logaritmo y despeja $y$.",
      },
    ],
    visuales: [
      {
