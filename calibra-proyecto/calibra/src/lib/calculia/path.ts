...
export interface NodoCaminoCalculia {
  ...
  estado: NodoEstado;
  // Fase C: distingue las 5 técnicas rápidas originales (independientes,
  // sin requiere_pro) del "Curso estructurado (Pro)" nuevo — ambos
  // conviven en la misma tabla techniques/problem_type='calculia'.
  requierePro: boolean;
...
// Camino de Aprender de Calculia — mismo patrón que
// src/lib/trigonometria/path.ts: reutiliza techniques/technique_progress
// (problem_type='calculia'), no una tabla nueva.
//
// Fase C: la tabla ahora tiene dos "recorridos" independientes dentro
// del mismo problem_type='calculia' — las 5 técnicas rápidas originales
// (requiere_pro=false, progresión secuencial de siempre) y el "Curso
// estructurado (Pro)" nuevo (requiere_pro=true). Cada uno tiene su
// PROPIA secuencia de desbloqueo independiente (completar la última
// técnica rápida no desbloquea el curso Pro ni viceversa). `esPro`
// determina el gating del curso Pro: el módulo 1 (orden más bajo entre
// las filas requiere_pro=true) es SIEMPRE accesible como preview
// gratuito: para un usuario no-Pro, los módulos 2+ quedan bloqueados
// con bloqueadoPorPlan=true sin importar el progreso real; para un
// usuario Pro, se aplica la misma progresión secuencial de siempre.
export async function obtenerCaminoCalculia(
  ...
  const nodosRapidas: NodoCaminoCalculia[] = rapidas.map((t, i) => ({
    id: t.id,
    slug: t.slug,
    nombre: t.nombre,
  ...
  const nodosCursoPro: NodoCaminoCalculia[] = cursoPro.map((t, i) => {
