...
import type { ModoEstadistica } from "@/lib/practica/estadistica";
import { MODOS_ESTADISTICA } from "@/lib/practica/estadistica";

export interface DueloEstadisticaInfo {
  duelId: string;
  rivalId: string;
  rivalNombre: string;
  miElo: number;
...
export interface DatosPracticaEstadistica {
  ...
  boostActivo: boolean;
  dueloInfo: DueloEstadisticaInfo | null;
}

const MODOS_VALIDOS: readonly ModoEstadistica[] = MODOS_ESTADISTICA;
...
export async function cargarDatosPracticaEstadistica(
  supabase: SupabaseClient,
  userId: string,
  modoDeLaRuta: ModoEstadistica,
  duelo?: string
): Promise<DatosPracticaEstadistica> {
  let dueloInfo: DueloEstadisticaInfo | null = null;
  ...
  const [{ data: nivelRow }, { data: profile }] = await Promise.all([
    supabase.from("skill_levels").select("nivel").eq("user_id", userId).eq("problem_type", `estadistica_${modo}`).maybeSingle(),
