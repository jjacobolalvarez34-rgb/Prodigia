...
import { MODOS_NAIPIA, type ModoNaipia } from "@/lib/practica/naipia";

export interface DueloNaipiaInfo {
  duelId: string;
  rivalId: string;
  rivalNombre: string;
  miElo: number;
...
export interface DatosPracticaNaipia {
  modo: ModoNaipia;
  nivelInicial: number;
  escudosExtra: number;
  ...
  boostActivo: boolean;
  dueloInfo: DueloNaipiaInfo | null;
}

const MODOS_VALIDOS: ModoNaipia[] = MODOS_NAIPIA;
...
export async function cargarDatosPracticaNaipia(
  supabase: SupabaseClient,
  userId: string,
  modoDeLaRuta: ModoNaipia,
  duelo?: string
): Promise<DatosPracticaNaipia> {
  let dueloInfo: DueloNaipiaInfo | null = null;
  ...
  const [{ data: nivelRow }, { data: profile }] = await Promise.all([
    supabase.from("skill_levels").select("nivel").eq("user_id", userId).eq("problem_type", `naipia_${modo}`).maybeSingle(),
