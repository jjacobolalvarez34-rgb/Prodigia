...
import { generarProblemaDecimal, type ProblemaDecimal } from "@/lib/practica/decimales";
...
export default function DecimalPracticaClient({ nivelInicial, escudosExtra, boostActivo }: Props) {
  ...
  const [errores, setErrores] = useState<ProblemaDecimal[]>([]);
  ...
  async function handleFinish(erroresPartida: ProblemaDecimal[]) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-4 py-20 text-center">
      ...
      <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">Decimales y porcentajes</h1>
      <p className="text-texto-secundario">Convertir, calcular porcentajes y redondear. 10 problemas o 60 segundos.</p>
