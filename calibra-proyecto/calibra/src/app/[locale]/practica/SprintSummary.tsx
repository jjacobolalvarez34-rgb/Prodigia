...
function Comparacion({
  ...
  const t = useTranslations("Practica.resumen");
  return (
    <div className="flex items-center justify-between border-b border-border py-3.5 last:border-0">
      ...
      <div className="flex items-baseline gap-2">
        ...
        <span className="text-xs text-texto-secundario">{t("vsTuPromedio", { historico })}</span>
      </div>
    </div>
  );
}
    ...
    <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 py-20">
      ...
      {resumen.metaAlcanzada && (
        <div className="rounded-full bg-logro/20 px-4 py-1.5 text-sm font-medium text-foreground">
          🎉 {t("rompisteTuMeta", { ganado: resumen.xpGanadoHoy, meta: resumen.metaXpDiaria })}
        </div>
      )}
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface px-6 py-4 shadow-sm">
        <Comparacion
          ...
          historico={hayHistorial ? pct(historico.precision) : sinHistorial}
        />
        <Comparacion
          label={t("precision")}
        ...
        />
        <Comparacion
          label={t("velocidadPromedio")}
          actual={segundos(sprint.avgTimeMs)}
          historico={hayHistorial ? segundos(historico.avgTimeMs) : sinHistorial}
        />
        <Comparacion
          label={t("experienciaHoy")}
          actual={`${resumen.xpGanadoHoy}`}
          historico={
            historico.avgXpDiario !== null ? Math.round(historico.avgXpDiario).toString() : sinHistorial
          }
        />
      </div>

      <p className="font-mono text-xs text-texto-secundario">{t("chispasTotales", { n: resumen.puntosTotal })}</p>
            ...
            {errores.map((p, i) => (
              <div key={i} className="flex items-center justify-between font-mono text-sm">
                ...
                <span className="text-texto-secundario">
                  {t("laRespuestaEra")} <span className="font-semibold text-error">{p.answer}</span>
                </span>
              </div>
            ))}
          </div>
