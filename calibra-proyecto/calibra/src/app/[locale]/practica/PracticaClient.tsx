...
export default function PracticaClient({
  ...
  async function handleFinishSprint(errores: Problem[], respuestas: RespuestaDuelo[]) {
    ...
    try {
      ...
      if (!res.ok) {
        setErrorResumen(data.error ?? t("noSePudoCerrar"));
        setFase("resumen");
        return;
      }
  ...
  // un duelo Ranked real (no contra el Clan de Bots, que ya no muestra el
  // botón de Rendirse) el modal avisa la pérdida de ELO y, al confirmar,
  // rinde el duelo de verdad (mismo POST que BotonRendirse) antes de
  ...
  if (fase === "resumen" && errorResumen) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 py-16 text-center">
        <p className="text-error">{t("noPudimosCerrar", { error: errorResumen })}</p>
        <Boton onClick={() => setFase("seleccion")}>{t("volver")}</Boton>
      </div>
    );
  }
