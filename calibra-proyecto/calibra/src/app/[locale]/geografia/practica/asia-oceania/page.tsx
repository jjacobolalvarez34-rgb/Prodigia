...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("Geografia.metadata.practicaAsiaOceania");
...
export default async function GeografiaPracticaAsiaOceaniaPage() {
  ...
  const [{ data: nivelRow }, { data: profile }] = await Promise.all([
    supabase.from("skill_levels").select("nivel").eq("user_id", user.id).eq("problem_type", "geografia_asia_oceania").maybeSingle(),
