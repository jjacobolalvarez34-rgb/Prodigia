...
import { generarProblemaCalculia, type ProblemaCalculiaOpciones } from "@/lib/practica/calculia";
...
function claveDerivadas(p: ProblemaCalculiaOpciones): string {
  return `${p.enunciado}|${p.respuesta}`;
}

// Mismo patrón que DiagnosticoHistoriaClient.tsx — preguntas del modo
// "derivadas" (siempre opción múltiple, ver calculia.ts), así que no
// hace falta el input numérico que sí necesita DiagnosticoTrigonometriaClient.tsx.
export default function DiagnosticoCalculiaClient({ destino }: Props) {
  const t = useTranslations("Calculia");
  const tInicio = useTranslations("Common.errorBoundario");
  const router = useRouter();
  const [fase, setFase] = useState<Fase>("intro");
  const [indice, setIndice] = useState(0);
  const [pregunta, setPregunta] = useState<ProblemaCalculiaOpciones | null>(null);
  ...
  function siguientePregunta() {
    const p = generarSinRepetir(
      () => generarProblemaCalculia("derivadas", nivelRef.current) as ProblemaCalculiaOpciones,
      claveDerivadas,
      usadosRef.current
    );
    setPregunta(p);
  ...
  async function guardar(nivel: number): Promise<boolean> {
    ...
    await supabase
      .from("skill_levels")
      .upsert({ user_id: user.id, problem_type: "calculia_derivadas", nivel, racha_actual: 0 }, { onConflict: "user_id,problem_type" });
    const { error } = await supabase.from("profiles").update({ onboarding_calculia_completado: true }).eq("id", user.id);
    return !error;
  }
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="intro" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6 text-center">
            <div>
              <span
                ...
                style={{ background: `color-mix(in oklab, ${COLOR_CALCULIA} 10%, transparent)`, color: COLOR_CALCULIA }}
              >
                {t("nombreMundo")}
              </span>
            ...
            <button
              ...
              style={{ background: `linear-gradient(120deg, ${COLOR_CALCULIA}, #818CF8)` }}
            >
          ...
          >
            <div className="flex gap-1.5">
              {Array.from({ length: TOTAL_PREGUNTAS }).map((_, i) => (
                <span key={i} className="h-1.5 w-5 rounded-full" style={{ background: i <= indice ? COLOR_CALCULIA : undefined }} />
              ))}
            </div>
                ...
                {pregunta.opciones.map((op) => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-xl border-2 px-4 py-3 text-left font-mono text-sm font-medium transition-colors disabled:opacity-100 ${
                        esCorrecta
        ...
        {fase === "resultado" && (
          <motion.div key="resultado" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-6 text-center">
            ...
            <p className="font-mono text-4xl font-bold" style={{ color: COLOR_CALCULIA }}>
            ...
            <button
              ...
              style={{ background: guardadoOk ? `linear-gradient(120deg, ${COLOR_CALCULIA}, #818CF8)` : "var(--error)" }}
            >
