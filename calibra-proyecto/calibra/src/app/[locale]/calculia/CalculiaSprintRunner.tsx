...
import { generarProblemaCalculia, type ModoCalculia, type ProblemaCalculia } from "@/lib/practica/calculia";
...
function claveCalculia(p: ProblemaCalculia): string {
...
interface Props {
  ...
  duracionMs?: number;
  onFinish: (errores: ProblemaCalculia[], correctos: number) => void;
}

// Mismo patrón que TrigonometriaSprintRunner.tsx (timer/escudos/racha/
// duelo en vivo, sin semilla, mezcla de entrada numérica con tolerancia
// y grilla de opciones dentro del mismo runner) — Calculia es el
// segundo mundo, después de Trigonometría, que necesita los 2 estilos
// de entrada en un solo componente (Series mezcla suma geométrica/
// criterio de la razón, ambos numéricos, con clasificación de
// convergencia, de opciones).
export default function CalculiaSprintRunner({
  modo,
  startedAt,
  nivelInicial,
  escudosExtra,
  ...
  const t = useTranslations("Calculia.sprintRunner");
  ...
  const erroresRef = useRef<ProblemaCalculia[]>([]);
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-4 py-12">
      <LevelDial nivel={nivel} size={72} colorHex={COLOR_CALCULIA} />
          ...
          <div className="flex gap-1.5">
            {Array.from({ length: totalPreguntas }).map((_, i) => (
              <span
                ...
                style={{ background: i < respondidos ? COLOR_CALCULIA : "var(--foreground)", opacity: i < respondidos ? 1 : 0.15 }}
              />
            ))}
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1" aria-label={t("escudosDisponibles", { n: escudos })}>
              {Array.from({ length: escudosIniciales }).map((_, i) => (
                <EscudoIcon key={i} activo={i < escudos} colorActivo={COLOR_CALCULIA} />
              ))}
            </div>
