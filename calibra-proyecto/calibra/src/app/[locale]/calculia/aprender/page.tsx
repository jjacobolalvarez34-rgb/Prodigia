...
export default async function CalculiaAprenderPage({ searchParams }: Props) {
  ...
  // filas techniques y misma lógica de obtenerCaminoCalculia: solo cambia
  ...
  const unidadClases: UnidadCaminoGenerico = {
    ...
    nodos: clases.map((n) => ({
      ...
      ctaPro: n.bloqueadoPorPlan
        ? { label: t("aprender.clases.desbloqueaConPro"), href: "/pro?next=%2Fcalculia%2Faprender%3Ftab%3Dclases" }
        : undefined,
    })),
  };
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-8 px-4 py-12 sm:px-6">
        ...
        >
          <AprenderTabs
            colorHex={COLOR_CALCULIA}
            esPro={esPro}
            proHref="/pro?next=%2Fcalculia%2Faprender%3Ftab%3Dclases"
            ...
            tecnicas={<CaminoContinuo unidades={[unidadTecnicas]} basePath="/calculia/aprender" colorHex={COLOR_CALCULIA} />}
            clases={
              hayClases ? (
                <CaminoContinuo unidades={[unidadClases]} basePath="/calculia/aprender" colorHex={COLOR_CALCULIA} />
              ) : null
            }
          />
        </AprenderLayout>
