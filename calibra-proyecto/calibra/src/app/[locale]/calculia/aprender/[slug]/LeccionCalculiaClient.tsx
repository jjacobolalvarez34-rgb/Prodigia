...
import type { NodoCaminoCalculia } from "@/lib/calculia/path";
...
// tiene preguntas (las "Clases" Pro de Calculia). Las
...
export default function LeccionCalculiaClient({ nodo }: Props) {
  const t = useTranslations("Calculia.leccion");
          ...
          <motion.div key="explicacion" {...transicion} className="flex flex-col gap-6 text-center">
            <div>
              <span
                ...
                style={{ background: `color-mix(in oklab, ${COLOR_CALCULIA} 12%, transparent)`, color: COLOR_CALCULIA }}
              >
                {t("tecnica")}
              </span>
        ...
        {fase === "celebracion" && (
          ...
          >
            ...
            <div className="mt-2 flex w-full flex-col gap-3">
              <button
                onClick={() => router.push(hrefVolverAAprender("/calculia/aprender", nodo.requierePro))}
                className="rounded-xl px-4 py-3 font-display font-semibold text-white"
                style={{ background: COLOR_CALCULIA }}
              >
                {t("volverAAprender")}
              </button>
              <Link href="/calculia/practica" className="text-sm font-medium hover:underline" style={{ color: COLOR_CALCULIA }}>
                {t("irAPracticar")}
              </Link>
            </div>
          </motion.div>
