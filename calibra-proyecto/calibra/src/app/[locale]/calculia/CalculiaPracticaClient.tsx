...
import type { ModoCalculia, ProblemaCalculia } from "@/lib/practica/calculia";
import { NOMBRE_MODO_CALCULIA } from "@/lib/practica/calculia";
import type { DueloCalculiaInfo } from "@/lib/calculia/cargarPractica";
...
// Mismo valor que TOTAL_PREGUNTAS en CalculiaSprintRunner.tsx (archivo
...
interface Props {
  ...
  boostActivo: boolean;
  duelo?: DueloCalculiaInfo | null;
  miUserId: string;
}

// Mismo patrón que TrigonometriaPracticaClient.tsx.
export default function CalculiaPracticaClient({ modo, nivelInicial, escudosExtra, hielosDisponibles, tiemposExtraDisponibles, boostActivo, duelo, miUserId }: Props) {
  ...
  const [errores, setErrores] = useState<ProblemaCalculia[]>([]);
  ...
  async function handleFinish(erroresPartida: ProblemaCalculia[]) {
  ...
  if (fase === "resumen" && error) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 py-16 text-center">
        ...
        <button onClick={() => setFase("inicio")} className="rounded-2xl px-4 py-3 font-medium text-white" style={{ background: COLOR_CALCULIA }}>
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 py-20">
        ...
        <BotonesFinPartida
          ...
          volverHref={duelo ? "/rankeds" : "/calculia"}
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-4 py-20 text-center">
      ...
      <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">{NOMBRE_MODO_CALCULIA[modo]}</h1>
      <p className="text-texto-secundario">{t("practicaClient.preguntasOSegundos")}</p>
      <Boton onClick={iniciar} colorHex={COLOR_CALCULIA} destacado className="w-full py-5 text-lg">
