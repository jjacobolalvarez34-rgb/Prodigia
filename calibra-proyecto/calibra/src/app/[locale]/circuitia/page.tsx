...
import { IconCheck, IconCircuitia } from "@/components/icons";
import { COLOR_CIRCUITIA } from "./colores";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("Circuitia.metadata");
  return { title: t("title"), description: t("description") };
}

const TIPOS_CIRCUITIA = ["circuitia_serie", "circuitia_paralelo", "circuitia_mixto", "circuitia_cualitativo"];

export default async function CircuitiaHomePage() {
  const t = await getTranslations("Circuitia");
  const supabase = await createClient();
  const { user, profile } = await requireMundoCircuitia(supabase, "/circuitia");

  const hoyIso = new Date().toISOString().slice(0, 10);
  const [{ data: nivelRows }, { data: dailyHoy }, { data: mundoProgreso }] = await Promise.all([
    supabase.from("skill_levels").select("problem_type, nivel").eq("user_id", user.id).in("problem_type", TIPOS_CIRCUITIA),
    supabase.from("daily_progress").select("xp_ganado").eq("user_id", user.id).eq("fecha", hoyIso).maybeSingle(),
    supabase.rpc("sincronizar_progreso_mundo", { p_world: "circuitia" }).returns<SincronizarProgresoRow[]>().maybeSingle(),
  ]);
  ...
  return (
    <>
      <FondoMundo mundo="circuitia" />
      <FondoCursorMundo mundo="circuitia" />
      <Header autenticado />
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-10 px-4 py-12 sm:px-6">
        <AvisoPrimeraVez avisoKey="circuitia-intro" texto={t("aviso")}>
          <div>
            ...
            <div className="mt-2">
              <NivelMundoBadge nombreMundo={t("nombreMundo")} nivel={nivelMundo} colorHex={COLOR_CIRCUITIA} />
            </div>
          </div>
        </AvisoPrimeraVez>

        <NivelMundoProgreso nombreMundo={t("nombreMundo")} colorHex={COLOR_CIRCUITIA} progreso={progresoMundo} />
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          <Link
            ...
            style={{
              borderColor: `color-mix(in oklab, ${COLOR_CIRCUITIA} 35%, transparent)`,
              background: `color-mix(in oklab, ${COLOR_CIRCUITIA} 7%, var(--surface))`,
            }}
          >
            <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ background: COLOR_CIRCUITIA }}>
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <TopicCard nombre={t("modos.serie")} Icono={IconCircuitia} badge={{ tipo: "nivel", nivel: nivelDe("circuitia_serie") }} colorHex={COLOR_CIRCUITIA} />
            <TopicCard nombre={t("modos.paralelo")} Icono={IconCircuitia} badge={{ tipo: "nivel", nivel: nivelDe("circuitia_paralelo") }} colorHex={COLOR_CIRCUITIA} />
            <TopicCard nombre={t("modos.mixto")} Icono={IconCircuitia} badge={{ tipo: "nivel", nivel: nivelDe("circuitia_mixto") }} colorHex={COLOR_CIRCUITIA} />
            <TopicCard nombre={t("modos.cualitativo")} Icono={IconCircuitia} badge={{ tipo: "nivel", nivel: nivelDe("circuitia_cualitativo") }} colorHex={COLOR_CIRCUITIA} />
          </div>
        </section>
      </div>
    </>
