...
import type { ModoCircuitia, ProblemaCircuitia } from "@/lib/practica/circuitia";
import { NOMBRE_MODO_CIRCUITIA } from "@/lib/practica/circuitia";
import type { DueloCircuitiaInfo } from "@/lib/circuitia/cargarPractica";
...
interface Props {
  ...
  boostActivo: boolean;
  duelo?: DueloCircuitiaInfo | null;
  miUserId: string;
}

// Mismo patrón que CalculiaPracticaClient.tsx.
export default function CircuitiaPracticaClient({ modo, nivelInicial, escudosExtra, hielosDisponibles, tiemposExtraDisponibles, boostActivo, duelo, miUserId }: Props) {
  ...
  const [errores, setErrores] = useState<ProblemaCircuitia[]>([]);
  ...
  async function handleFinish(erroresPartida: ProblemaCircuitia[]) {
  ...
  if (fase === "resumen" && error) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 py-16 text-center">
        ...
        <button onClick={() => setFase("inicio")} className="rounded-2xl px-4 py-3 font-medium text-white" style={{ background: COLOR_CIRCUITIA }}>
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 py-20">
        ...
        <BotonesFinPartida
          ...
          volverHref={duelo ? "/rankeds" : "/circuitia"}
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-4 py-20 text-center">
      ...
      <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">{NOMBRE_MODO_CIRCUITIA[modo]}</h1>
      <p className="text-texto-secundario">{t("practicaClient.preguntasOSegundos")}</p>
      <Boton onClick={iniciar} colorHex={COLOR_CIRCUITIA} destacado className="w-full py-5 text-lg">
