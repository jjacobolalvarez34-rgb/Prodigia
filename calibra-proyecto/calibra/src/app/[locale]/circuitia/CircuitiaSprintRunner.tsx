...
import { generarProblemaCircuitia, type ModoCircuitia, type ProblemaCircuitia } from "@/lib/practica/circuitia";
...
function claveCircuitia(p: ProblemaCircuitia): string {
...
interface Props {
  ...
  duracionMs?: number;
  onFinish: (errores: ProblemaCircuitia[], correctos: number) => void;
}

// Mismo patrón que CalculiaSprintRunner.tsx/TrigonometriaSprintRunner.tsx
// (timer/escudos/racha/duelo en vivo, sin semilla, mezcla de entrada
// numérica con tolerancia y grilla de opciones dentro del mismo
// runner) — a diferencia de esos 2, acá cada problema trae un diagrama
// propio (CircuitoSVG), calculado desde la topología estructurada que
// devuelve el generador, nunca un glifo suelto.
export default function CircuitiaSprintRunner({
  modo,
  startedAt,
  nivelInicial,
  escudosExtra,
  ...
  const t = useTranslations("Circuitia.sprintRunner");
  ...
  const [problema, setProblema] = useState<ProblemaCircuitia | null>(null);
  ...
  const erroresRef = useRef<ProblemaCircuitia[]>([]);
  const shownAtRef = useRef(0);
  const submittingRef = useRef(false);
  const finishedRef = useRef(false);

  function siguiente() {
    const p = generarSinRepetir(() => generarProblemaCircuitia(modo, nivelRef.current), claveCircuitia, usadosRef.current);
    setProblema(p);
    setCardKey((k) => k + 1);
    setRespuestaTexto("");
    setSeleccion(null);
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-4 py-12">
      <LevelDial nivel={nivel} size={72} colorHex={COLOR_CIRCUITIA} />
          ...
          <div className="flex gap-1.5">
            {Array.from({ length: totalPreguntas }).map((_, i) => (
              <span
                ...
                style={{ background: i < respondidos ? COLOR_CIRCUITIA : "var(--foreground)", opacity: i < respondidos ? 1 : 0.15 }}
              />
            ))}
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1" aria-label={t("escudosDisponibles", { n: escudos })}>
              {Array.from({ length: escudosIniciales }).map((_, i) => (
                <EscudoIcon key={i} activo={i < escudos} colorActivo={COLOR_CIRCUITIA} />
              ))}
            </div>
