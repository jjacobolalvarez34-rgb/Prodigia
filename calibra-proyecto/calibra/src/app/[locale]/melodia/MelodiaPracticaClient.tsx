...
import MelodiaSprintRunner from "./MelodiaSprintRunner";
import { COLOR_MELODIA } from "./colores";
import { estadoResumenPartida } from "@/lib/practica/resumenPartida";

// Mismo valor que TOTAL_PREGUNTAS en MelodiaSprintRunner.tsx (archivo
...
export default function MelodiaPracticaClient({ modo, nivelInicial, escudosExtra, hielosDisponibles, tiemposExtraDisponibles, boostActivo, duelo, miUserId }: Props) {
  ...
  const [errores, setErrores] = useState<PreguntaMelodia[]>([]);
  ...
  async function handleFinish(erroresPartida: PreguntaMelodia[]) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? t("errorCierre"));
        setFase("resumen");
        return;
      }
      setError(null);
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 py-20">
        ...
        <div className="flex flex-col items-center gap-2 text-center">
          <p className="font-display text-lg font-bold text-foreground">{t("resumenTitulo")}</p>
        ...
        {errores.length > 0 ? (
          <div className="w-full max-w-md rounded-2xl border border-border bg-surface px-6 py-4 shadow-sm">
            <p className="mb-3 font-display text-sm font-semibold text-foreground">{t("repasemos")}</p>
        ...
        <BotonesFinPartida
          ...
          volverHref={duelo ? "/rankeds" : "/melodia"}
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-4 py-20 text-center">
      ...
      <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">{NOMBRE_MODO_MELODIA[modo]}</h1>
      <p className="text-texto-secundario">{t("instrucciones")}</p>
      <Boton onClick={iniciar} colorHex={COLOR_MELODIA} destacado className="w-full py-5 text-lg">
        {t("iniciarPartida")}
      </Boton>
    </div>
  );
