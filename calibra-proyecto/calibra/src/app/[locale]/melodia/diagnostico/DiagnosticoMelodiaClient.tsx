...
export default function DiagnosticoMelodiaClient({ destino }: Props) {
  ...
  function siguientePregunta() {
    const p = generarSinRepetir(() => generarPreguntaMelodia("fundamentos", nivelRef.current), claveMelodia, usadosRef.current);
    setPregunta(p);
    setSeleccion(null);
    setFeedback("idle");
          ...
          <motion.div key="intro" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6 text-center">
            <div>
              <span
                ...
                style={{ background: `color-mix(in oklab, ${COLOR_MELODIA} 10%, transparent)`, color: COLOR_MELODIA }}
              >
                Melodía
              </span>
          ...
          >
            <div className="flex gap-1.5">
              {Array.from({ length: TOTAL_PREGUNTAS }).map((_, i) => (
                <span key={i} className="h-1.5 w-5 rounded-full" style={{ background: i <= indice ? COLOR_MELODIA : undefined }} />
              ))}
            </div>
