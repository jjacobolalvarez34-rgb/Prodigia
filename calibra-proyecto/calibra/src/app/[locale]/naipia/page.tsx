...
import { IconCheck, IconNaipia } from "@/components/icons";
import { COLOR_NAIPIA } from "./colores";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("Naipia.metadata");
  return { title: t("title"), description: t("description") };
}

const TIPOS_NAIPIA = ["naipia_hilo", "naipia_ko", "naipia_hiopt2", "naipia_omega2", "naipia_verdadero"];

export default async function NaipiaHomePage() {
  const t = await getTranslations("Naipia");
  const supabase = await createClient();
  const { user, profile } = await requireMundoNaipia(supabase, "/naipia");

  const hoyIso = new Date().toISOString().slice(0, 10);
  const [{ data: nivelRows }, { data: dailyHoy }, { data: mundoProgreso }] = await Promise.all([
    supabase.from("skill_levels").select("problem_type, nivel").eq("user_id", user.id).in("problem_type", TIPOS_NAIPIA),
    supabase.from("daily_progress").select("xp_ganado").eq("user_id", user.id).eq("fecha", hoyIso).maybeSingle(),
    supabase.rpc("sincronizar_progreso_mundo", { p_world: "naipia" }).returns<SincronizarProgresoRow[]>().maybeSingle(),
  ]);
  ...
  return (
    <>
      <FondoMundo mundo="naipia" />
      <FondoCursorMundo mundo="naipia" />
      <Header autenticado />
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-10 px-4 py-12 sm:px-6">
        <AvisoPrimeraVez avisoKey="naipia-intro" texto={t("aviso")}>
          <div>
            <span className="text-xs font-medium uppercase tracking-wide" style={{ color: COLOR_NAIPIA }}>
              {t("nombreMundo")}
            </span>
            <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">{t("titulo")}</h1>
            <div className="mt-2">
              <NivelMundoBadge nombreMundo={t("nombreMundo")} nivel={nivelMundo} colorHex={COLOR_NAIPIA} />
            </div>
          </div>
        </AvisoPrimeraVez>

        <NivelMundoProgreso nombreMundo={t("nombreMundo")} colorHex={COLOR_NAIPIA} progreso={progresoMundo} />
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          <Link
            ...
            style={{
              borderColor: `color-mix(in oklab, ${COLOR_NAIPIA} 35%, transparent)`,
              background: `color-mix(in oklab, ${COLOR_NAIPIA} 7%, var(--surface))`,
            }}
          >
            <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ background: COLOR_NAIPIA }}>
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <TopicCard nombre={t("modos.hilo")} Icono={IconNaipia} badge={{ tipo: "nivel", nivel: nivelDe("naipia_hilo") }} colorHex={COLOR_NAIPIA} />
            <TopicCard nombre={t("modos.ko")} Icono={IconNaipia} badge={{ tipo: "nivel", nivel: nivelDe("naipia_ko") }} colorHex={COLOR_NAIPIA} />
            <TopicCard nombre={t("modos.hiopt2")} Icono={IconNaipia} badge={{ tipo: "nivel", nivel: nivelDe("naipia_hiopt2") }} colorHex={COLOR_NAIPIA} />
            <TopicCard nombre={t("modos.omega2")} Icono={IconNaipia} badge={{ tipo: "nivel", nivel: nivelDe("naipia_omega2") }} colorHex={COLOR_NAIPIA} />
            <TopicCard nombre={t("modos.verdadero")} Icono={IconNaipia} badge={{ tipo: "nivel", nivel: nivelDe("naipia_verdadero") }} colorHex={COLOR_NAIPIA} />
          </div>
        </section>
      </div>
    </>
