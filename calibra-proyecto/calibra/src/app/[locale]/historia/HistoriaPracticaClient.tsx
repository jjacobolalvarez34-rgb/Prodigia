...
import HistoriaSprintRunner from "./HistoriaSprintRunner";
import { COLOR_HISTORIA } from "./colores";
import { estadoResumenPartida } from "@/lib/practica/resumenPartida";

// Mismo valor que TOTAL_PREGUNTAS en HistoriaSprintRunner.tsx (archivo
...
export default function HistoriaPracticaClient({ modo, nivelInicial, escudosExtra, hielosDisponibles, tiemposExtraDisponibles, boostActivo, duelo, miUserId }: Props) {
  ...
  const [errores, setErrores] = useState<PreguntaHistoria[]>([]);
  ...
  async function handleFinish(erroresPartida: PreguntaHistoria[]) {
  ...
  if (fase === "resumen" && error) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 py-16 text-center">
        ...
        <Boton atras colorHex={COLOR_HISTORIA} onClick={() => setFase("inicio")}>
          {t("volver")}
        </Boton>
      </div>
    );
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 py-20">
        ...
        <BotonesFinPartida
          ...
          volverHref={duelo ? "/rankeds" : "/historia"}
