...
// modo "cronologia" (el modo de entrada), 100% opción múltiple igual
...
export default function DiagnosticoHistoriaClient({ destino }: Props) {
  ...
  function siguientePregunta() {
    const p = generarPreguntaHistoria("cronologia", nivelRef.current, usadosRef.current);
  ...
  async function guardar(nivel: number): Promise<boolean> {
    ...
    await supabase
      .from("skill_levels")
      .upsert({ user_id: user.id, problem_type: "historia_cronologia", nivel, racha_actual: 0 }, { onConflict: "user_id,problem_type" });
    const { error } = await supabase.from("profiles").update({ onboarding_historia_completado: true }).eq("id", user.id);
    return !error;
  }
          ...
          <motion.div key="intro" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6 text-center">
            <div>
              <span
                ...
                style={{ background: `color-mix(in oklab, ${COLOR_HISTORIA} 10%, transparent)`, color: COLOR_HISTORIA }}
              >
                Historia
              </span>
          ...
          >
            <div className="flex gap-1.5">
              {Array.from({ length: TOTAL_PREGUNTAS }).map((_, i) => (
                <span key={i} className="h-1.5 w-5 rounded-full" style={{ background: i <= indice ? COLOR_HISTORIA : undefined }} />
              ))}
            </div>
