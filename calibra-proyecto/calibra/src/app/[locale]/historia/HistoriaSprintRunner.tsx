...
// 100% opción múltiple (contenido curado a mano, no procedural), así
// que no hace falta el branching por tipo de entrada que sí necesita
// TrigonometriaSprintRunner.tsx. Botones en una sola columna (no grid
// de 2) porque las opciones de Historia son oraciones largas ("A → B →
// C" en el modo Cronología, o una consecuencia completa en Causa y
// efecto) — un grid de 2 las recortaría.
export default function HistoriaSprintRunner({
  modo,
  startedAt,
  nivelInicial,
  escudosExtra,
  ...
  const erroresRef = useRef<PreguntaHistoria[]>([]);
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-4 py-12">
      <LevelDial nivel={nivel} size={72} colorHex={COLOR_HISTORIA} />
          ...
          <div className="flex gap-1.5">
            {Array.from({ length: totalPreguntas }).map((_, i) => (
              <span
                ...
                style={{ background: i < respondidos ? COLOR_HISTORIA : "var(--foreground)", opacity: i < respondidos ? 1 : 0.15 }}
              />
            ))}
          </div>
          ...
          {pregunta.opciones.map((op) => {
            ...
            return (
              <button
                ...
                disabled={respondido}
                className={`rounded-xl border-2 px-4 py-3 text-left text-sm font-medium transition-colors disabled:opacity-100 ${
                  esCorrecta
