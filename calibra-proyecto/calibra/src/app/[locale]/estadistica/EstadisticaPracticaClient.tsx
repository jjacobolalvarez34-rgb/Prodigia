...
import type { ModoEstadistica, ProblemaEstadistica } from "@/lib/practica/estadistica";
import { NOMBRE_MODO_ESTADISTICA } from "@/lib/practica/estadistica";
import type { DueloEstadisticaInfo } from "@/lib/estadistica/cargarPractica";
...
// Mismo valor que TOTAL_PREGUNTAS en EstadisticaSprintRunner.tsx (archivo
...
interface Props {
  ...
  boostActivo: boolean;
  duelo?: DueloEstadisticaInfo | null;
  miUserId: string;
}

// Mismo patrón que CalculiaPracticaClient.tsx / TrigonometriaPracticaClient.tsx.
export default function EstadisticaPracticaClient({ modo, nivelInicial, escudosExtra, hielosDisponibles, tiemposExtraDisponibles, boostActivo, duelo, miUserId }: Props) {
  ...
  const [errores, setErrores] = useState<ProblemaEstadistica[]>([]);
  ...
  async function handleFinish(erroresPartida: ProblemaEstadistica[]) {
  ...
  if (fase === "resumen" && error) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 py-16 text-center">
        ...
        <button onClick={() => setFase("inicio")} className="rounded-2xl px-4 py-3 font-medium text-white" style={{ background: COLOR_ESTADISTICA }}>
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 py-20">
        ...
        <BotonesFinPartida
          ...
          volverHref={duelo ? "/rankeds" : "/estadistica"}
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-4 py-20 text-center">
      ...
      <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">{NOMBRE_MODO_ESTADISTICA[modo]}</h1>
      <p className="text-texto-secundario">{t("practicaClient.preguntasOSegundos")}</p>
      <Boton onClick={iniciar} colorHex={COLOR_ESTADISTICA} destacado className="w-full py-5 text-lg">
