...
import { IconCheck, IconEstadistica } from "@/components/icons";
import { COLOR_ESTADISTICA } from "./colores";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("Estadistica.metadata");
...
export default async function EstadisticaHomePage() {
  const t = await getTranslations("Estadistica");
  const supabase = await createClient();
  const { user, profile } = await requireMundoEstadistica(supabase, "/estadistica");

  const hoyIso = new Date().toISOString().slice(0, 10);
  const [{ data: nivelRows }, { data: dailyHoy }, { data: mundoProgreso }] = await Promise.all([
    supabase.from("skill_levels").select("problem_type, nivel").eq("user_id", user.id).in("problem_type", TIPOS_ESTADISTICA),
    supabase.from("daily_progress").select("xp_ganado").eq("user_id", user.id).eq("fecha", hoyIso).maybeSingle(),
    supabase.rpc("sincronizar_progreso_mundo", { p_world: "estadistica" }).returns<SincronizarProgresoRow[]>().maybeSingle(),
  ]);
  ...
  return (
    <>
      <FondoMundo mundo="estadistica" />
      <FondoCursorMundo mundo="estadistica" />
      <Header autenticado />
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-10 px-4 py-12 sm:px-6">
        <AvisoPrimeraVez avisoKey="estadistica-intro" texto={t("aviso")}>
          <div>
            <span className="text-xs font-medium uppercase tracking-wide" style={{ color: COLOR_ESTADISTICA }}>
              {t("nombreMundo")}
            </span>
            <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">{t("titulo")}</h1>
            <div className="mt-2">
              <NivelMundoBadge nombreMundo={t("nombreMundo")} nivel={nivelMundo} colorHex={COLOR_ESTADISTICA} />
            </div>
          </div>
        </AvisoPrimeraVez>

        <NivelMundoProgreso nombreMundo={t("nombreMundo")} colorHex={COLOR_ESTADISTICA} progreso={progresoMundo} />
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          <Link
            ...
            style={{
              borderColor: `color-mix(in oklab, ${COLOR_ESTADISTICA} 35%, transparent)`,
              background: `color-mix(in oklab, ${COLOR_ESTADISTICA} 7%, var(--surface))`,
            }}
          >
            <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ background: COLOR_ESTADISTICA }}>
              <IconEstadistica className="h-5 w-5" />
            </span>
            <div>
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <TopicCard nombre={t("modos.central")} Icono={IconEstadistica} badge={{ tipo: "nivel", nivel: nivelDe("estadistica_central") }} colorHex={COLOR_ESTADISTICA} />
            <TopicCard nombre={t("modos.dispersion")} Icono={IconEstadistica} badge={{ tipo: "nivel", nivel: nivelDe("estadistica_dispersion") }} colorHex={COLOR_ESTADISTICA} />
            <TopicCard nombre={t("modos.probabilidad")} Icono={IconEstadistica} badge={{ tipo: "nivel", nivel: nivelDe("estadistica_probabilidad") }} colorHex={COLOR_ESTADISTICA} />
            <TopicCard nombre={t("modos.datos")} Icono={IconEstadistica} badge={{ tipo: "nivel", nivel: nivelDe("estadistica_datos") }} colorHex={COLOR_ESTADISTICA} />
            <TopicCard nombre={t("modos.graficos")} Icono={IconEstadistica} badge={{ tipo: "nivel", nivel: nivelDe("estadistica_graficos") }} colorHex={COLOR_ESTADISTICA} />
          </div>
        </section>
      </div>
    </>
