...
import { IconEstadistica } from "@/components/icons";
import { COLOR_ESTADISTICA } from "../colores";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("Estadistica.elegir.metadata");
...
export default async function EstadisticaElegirPage() {
  const t = await getTranslations("Estadistica");
  const supabase = await createClient();
  const { user } = await requireMundoEstadistica(supabase, "/estadistica/elegir");
  ...
  const modos = [
    { nombre: t("modos.central"), desc: t("elegir.descripciones.central"), href: "/estadistica/practica", nivel: nivelDe("estadistica_central") },
    { nombre: t("modos.dispersion"), desc: t("elegir.descripciones.dispersion"), href: "/estadistica/practica/dispersion", nivel: nivelDe("estadistica_dispersion") },
    { nombre: t("modos.probabilidad"), desc: t("elegir.descripciones.probabilidad"), href: "/estadistica/practica/probabilidad", nivel: nivelDe("estadistica_probabilidad") },
    { nombre: t("modos.datos"), desc: t("elegir.descripciones.datos"), href: "/estadistica/practica/datos", nivel: nivelDe("estadistica_datos") },
    { nombre: `${t("modos.graficos")} 📊`, desc: t("elegir.descripciones.graficos"), href: "/estadistica/practica/graficos", nivel: nivelDe("estadistica_graficos") },
  ];
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {modos.map((m) => (
            ...
            >
              ...
              <LevelDial nivel={m.nivel} size={44} mostrarEtiqueta={false} colorHex={COLOR_ESTADISTICA} />
            </Link>
          ))}
        </div>
      </div>
