...
import type { ModoCodia, ProblemaCodia } from "@/lib/practica/codia";
import { NOMBRE_MODO_CODIA } from "@/lib/practica/codia";
import type { DueloCodiaInfo } from "@/lib/codia/cargarPractica";
...
export default function CodiaPracticaClient({ modo, nivelInicial, escudosExtra, hielosDisponibles, tiemposExtraDisponibles, boostActivo, duelo, miUserId }: Props) {
  ...
  const [errores, setErrores] = useState<ProblemaCodia[]>([]);
  ...
  async function handleFinish(erroresPartida: ProblemaCodia[]) {
  ...
  if (fase === "resumen" && error) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 py-16 text-center">
        ...
        <button onClick={() => setFase("inicio")} className="rounded-2xl px-4 py-3 font-medium text-white" style={{ background: COLOR_CODIA }}>
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 py-20">
        ...
        {errores.length > 0 ? (
          <div className="w-full max-w-md rounded-2xl border border-border bg-surface px-6 py-4 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {errores.map((p, i) => (
                <span key={i} className="whitespace-pre-line rounded-lg bg-surface-2 px-3 py-1 font-mono text-sm text-foreground">
                  {p.respuesta}
                </span>
              ))}
            </div>
        ...
        <BotonesFinPartida
          ...
          volverHref={duelo ? "/rankeds" : "/codia"}
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-4 py-20 text-center">
      ...
      <span className="font-mono text-4xl" style={{ color: COLOR_CODIA }} aria-hidden>{"</>"}</span>
      <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">{NOMBRE_MODO_CODIA[modo]}</h1>
      <p className="text-texto-secundario">{t("practicaClient.preguntasOSegundos")}</p>
      <Boton onClick={iniciar} colorHex={COLOR_CODIA} destacado className="w-full py-5 text-lg">
