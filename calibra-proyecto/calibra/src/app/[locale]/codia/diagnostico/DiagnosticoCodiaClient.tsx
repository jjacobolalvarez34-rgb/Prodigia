...
import { generarProblemaCodia, type ProblemaCodia } from "@/lib/practica/codia";
...
function claveProblema(p: ProblemaCodia): string {
  return `${p.lenguaje}|${p.enunciado}|${p.codigo}|${p.respuesta}`;
}

// Mismo patrón que DiagnosticoCalculiaClient — preguntas del modo "salida"
// (banda 3-6, la que mejor separa niveles; siempre opción múltiple) que
...
export default function DiagnosticoCodiaClient({ destino }: Props) {
  ...
  const [pregunta, setPregunta] = useState<ProblemaCodia | null>(null);
  ...
  function siguientePregunta() {
    const p = generarSinRepetir(() => generarProblemaCodia("salida", nivelRef.current), claveProblema, usadosRef.current);
    setPregunta(p);
    setSeleccion(null);
    setFeedback("idle");
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="intro" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6 text-center">
            <div>
              <span
                ...
                style={{ background: `color-mix(in oklab, ${COLOR_CODIA} 10%, transparent)`, color: COLOR_CODIA }}
              >
                {t("nombreMundo")}
              </span>
          ...
          >
            <div className="flex gap-1.5">
              {Array.from({ length: TOTAL_PREGUNTAS }).map((_, i) => (
                <span key={i} className="h-1.5 w-5 rounded-full" style={{ background: i <= indice ? COLOR_CODIA : undefined }} />
              ))}
            </div>
            <div className="flex w-full flex-col items-center gap-6 rounded-3xl border-2 border-border bg-surface px-8 py-10">
              <InsigniaLenguaje lenguaje={pregunta.lenguaje} />
              <p className="text-center font-medium text-foreground">{pregunta.enunciado}</p>
              {pregunta.codigo && <BloqueCodigo codigo={pregunta.codigo} lenguaje={pregunta.lenguaje} numeros={false} />}
        ...
        {fase === "resultado" && (
          <motion.div key="resultado" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-6 text-center">
            ...
            <p className="font-mono text-4xl font-bold" style={{ color: COLOR_CODIA }}>
