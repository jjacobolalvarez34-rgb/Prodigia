  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-1 flex-col gap-10 px-4 py-12 sm:px-6">
      ...
      {!buscando && !error && consulta.trim().length >= 2 && resultados.length === 0 && (
        <p className="text-sm text-texto-secundario">{t("noEncontramosANadie")}</p>
      )}

      {resultados.length > 0 && (
        <section className="flex flex-col gap-2">
          <h2 className="text-xs font-medium uppercase tracking-wide text-texto-secundario">{t("resultados")}</h2>
          {resultados.map((r) => (
            <div key={r.id} className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-3">
              <span className="font-medium text-foreground">{r.display_name ?? t("jugador")}</span>
              <Boton onClick={() => enviarSolicitud(r.id)} disabled={enviadas.has(r.id)} className="px-3 py-1.5 text-sm">
                {enviadas.has(r.id) ? t("enviada") : t("enviarSolicitud")}
              </Boton>
            </div>
          ))}
        </section>
      ...
      {solicitudes.length > 0 && (
        <section className="flex flex-col gap-2">
          <h2 className="text-xs font-medium uppercase tracking-wide text-texto-secundario">{t("solicitudesPendientes")}</h2>
          {solicitudes.map((s) => (
            <div key={s.user_id} className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-3">
              <span className="font-medium text-foreground">{s.display_name ?? t("jugador")}</span>
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-xs font-medium uppercase tracking-wide text-texto-secundario">{t("tusAmigos")}</h2>
        {amigos.length === 0 ? (
...
function InvitarPorLink() {
  ...
  if (estado === "esperando" && link) {
    return (
      <div className="flex flex-col items-center gap-4 rounded-2xl border-2 border-primario/30 bg-primario/5 px-6 py-10 text-center">
        ...
        <div>
          <p className="font-display text-lg font-bold text-foreground">{t("esperandoQueSeUnan")}</p>
          <p className="mt-1 text-xs text-texto-secundario">
            {seleccion ? `${nombreMundo(seleccion.mundo)} · ${etiquetaOpcion(seleccion.mundo, seleccion.opcion)}` : ""} · {t("noHaceFaltaQueSeaAmigo")}
          </p>
        </div>
        <div className="flex w-full max-w-sm items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2">
          ...
          <button onClick={copiar} className="shrink-0 rounded-lg bg-primario px-3 py-1.5 text-xs font-semibold text-white">
            {copiado ? t("copiadoCheck") : t("copiar")}
          </button>
        </div>
        <button
          onClick={cancelar}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        {estado === "generando" ? t("generando") : t("generarLinkDeInvitacion")}
      </button>
      <p className="text-center text-xs text-texto-secundario">{t("compartiseloAQuienQuieras")}</p>
    </div>
  );
}
...
function InvitarAmigoSinCuenta() {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-2xl border border-border bg-surface px-5 py-4">
      <p className="text-sm text-texto-secundario">{t("quienSeRegistre")}</p>
