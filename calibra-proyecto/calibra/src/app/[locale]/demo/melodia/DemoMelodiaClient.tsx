...
export default function DemoMelodiaClient() {
  const [fase, setFase] = useState<Fase>("sprint");
  const [correctos, setCorrectos] = useState(0);
  const [startedAt] = useState(() => performance.now());

  function handleFinish(_errores: PreguntaMelodia[], correctosReales: number) {
  ...
  if (fase === "resultado") {
    return (
      <FlujoResultado correctos={correctos} total={TOTAL_DEMO} colorHex={COLOR_MELODIA} onContinuar={() => setFase("tour")} />
    );
  }
  if (fase === "tour") {
