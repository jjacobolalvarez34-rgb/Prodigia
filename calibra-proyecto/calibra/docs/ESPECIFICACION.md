...
## Convención fija: español neutro (sin voseo) — se aplica a TODO texto, en absolutamente cualquier cosa

**Regla vinculante, sin excepción, para cualquier texto visible de la app** — mensajes de UI, lecciones (`techniques.contenido.pasos`/`quiz`), anuncios, logros, copy de marketing dentro de la app, mensajes de error, nombres de botones: **tuteo neutro latinoamericano, nunca voseo rioplatense**. "Puedes", "responde", "sigue", "haz", "eres", "tú" — nunca "podés", "respondé", "seguí", "hacé", "sos", "vos". Pedido explícito del usuario (2026-09-22), tras encontrar voseo colado en varias secciones ("hacé", "seguí") tanto en la UI como en lecciones nuevas escritas por agentes de esta sesión que no conocían la regla.

- **Por qué se cuela tan fácil**: el patrón conversacional de Claude (y de gran parte del español "natural" con el que se entrena) tiende al voseo/tuteo mixto sin que se note al escribir una frase suelta — el problema aparece recién cuando se junta con el resto del texto del proyecto. Cualquier agente o sesión que escriba contenido nuevo (lecciones, mensajes, anuncios) tiene que releer lo que escribió pensando específicamente en esto, no asumir que "suena bien" alcanza.
- **Herramienta de verificación, ya construida y en la suite de tests**: `src/lib/texto/espanolNeutro.ts` (`detectarVoseo`/`detectarVoseoEnMensajes`) + `src/lib/texto/espanolNeutro.test.ts` — escanea automáticamente **todo `messages/es.json`** y **el contenido de texto de toda migración que haga `insert`/`update` sobre `techniques`, `logic_techniques`, `anuncios` o `achievements`** (nunca el DDL/RPCs, para no confundir identificadores de columna con prosa). Corré `npx vitest run src/lib/texto/espanolNeutro.test.ts` antes de dar por cerrada cualquier tanda de contenido nuevo — si falla, ahí están el término y el contexto exactos.
- **Scripts complementarios ya existentes** (anteriores a esta sesión, mismo objetivo): `scripts/detectar-voseo.mjs` (reporta, no corrige, solo sobre `messages/es.json`), `scripts/normalizar-espanol.mjs` (corrige `messages/es.json` con una tabla de reemplazos palabra por palabra, preservando mayúsculas), `scripts/analizar-voseo-funciones.mjs` (solo sobre los `raise exception` de funciones SQL). El test nuevo es más amplio: es el único que cubre el CONTENIDO de lecciones sembrado por `insert`, que es donde más volumen de texto nuevo se agrega por mundo y donde nadie había mirado hasta ahora.
- **Deuda histórica, documentada y acotada a propósito** (no se arregló en esta tanda, ver `DEUDA_HISTORICA_VOSEO` en `espanolNeutro.test.ts`): 33 migraciones desde `0005` hasta `0185` (casi toda la historia del proyecto, de antes de que esta regla se hiciera explícita) tienen voseo real en el contenido de sus lecciones — YA APLICADAS a la base real, así que editar esos archivos no cambia nada en producción; corregirlas de verdad exige una migración `UPDATE` nueva por cada una, fila por fila — un trabajo de contenido en sí mismo, del mismo tamaño que el Proceso 1/2 de `docs/PLAN_REVISION_CONTENIDO.md`. Quedan explícitamente exceptuadas del test (para que la suite no quede roja por deuda vieja) pero CUALQUIER migración nueva, o cualquier edición a una de esas 33, tiene que pasar el chequeo igual — sacarla de la lista de excepciones en cuanto se le escriba su corrección. Se corrigió en esta misma tanda (por estar sin aplicar todavía, editables en el lugar) el contenido nuevo de `0189`/`0191`/`0193`/`0195` (mundos 11-13 + KaTeX de Calculia/Circuitia) y todo `messages/es.json`.
