...
- **F14/15+F16 (2026-09-09, cierre)**: F14/15 (consolidacion transversal) se probaron y se devolvieron los pendientes detectados al informe final (no se creo PENDIENTES-CONSOLIDADOS.md separado). Se escribio `docs/FINAL-SPRINT-REPORT.md`: indice de 15 audits, PASO 1 (aplicar 0116→0128 en orden + `NOTIFY pgrst, 'reload schema';`), PASO 2 (retest por tunnel, 9 flujos A-I con checklist), PASO 3 (plantilla de reporte), RIESGOS (S5 doble-o-nada, S9, y causa raiz del 0 XP = RPC `insertar_intento`/`insertar_intento_logica` de 0120 NO aplicada a la base), y nota de ROLLBACK del badge F6 (no reintroducir).
- **0120 FALLIDA en prod del usuario (2026-09-09)**: error `42704 policy "usuarios insertan sus propios intentos" for table "attempts" does not exist`.
  CAUSA: los 5 `drop policy` de 0120 (lineas 33-41) no usan `if exists`, y en esa base las policies no existen con ese nombre. Nada de 0120 llego a aplicarse (las sentencias vienen SOLO despues de esas drops).
  SOLUCION: `supabase/migrations/0129_resto_sprint_0120_a_0128.sql` (GENERADA): concatenacion 0120 (con drop policy IF EXISTS) + 0121→0128 + `NOTIFY pgrst, 'reload schema';`. Idempotente en drops/create table; correr UNA sola vez. Verificada: 9/9 secciones presentes, 0 drop policy sin if exists.
- **0128 con bug de sintaxis (2026-09-09)**: el archivo fuente tenia 35 lineas de 'diff doc' (ej: `'no autenticado  →  'no autenticado`) SIN comentar, que rompen el parser (ERROR 42601) en cualquier editor. CORREGIDO: comentadas las lineas con U+2192 en `0128_espanol_neutro.sql` y regenerada `0129_resto_sprint_0120_a_0128.sql` (verificado: 0 lineas sueltas en ambos). Como la ejecucion del usuario fallo al INICIO de la seccion 0128 (0120→0127 ya quedaron aplicadas), el camino es correr SOLO el 0128 corregido + `NOTIFY pgrst, 'reload schema';` (no re-ejecutar la 0129 completa: sus seeds chocarian con filas ya insertadas). Nota: 0128 recrea 9 funciones y no necesita grants (create or replace conserva permisos).
- **Segunda falla de la unificada (2026-09-09)**: usuario re-ejecuto la 0129 y fallo con `42710 policy "trastienda_ruleta: lectura propia" already exists` (la seccion 0121 no es idempotente). DIAGNOSTICO: la base ya tenia 0120→0127 aplicadas (primera corrida); ademas las tablas de trastienda NO tienen primary key (id uuid default gen_random_uuid()) → re-ejecutar la 0129 duplicaria seeds. SOLUCION: `supabase/migrations/0130_0128_espanol_neutro_y_notify.sql` = SOLO 0128 (9 funciones neutro, idempotente) + NOTIFY. Instruido: NO re-ejecutar 0129/0120-0127.

- **Level-up de cuenta REINTRODUCIDO (2026-09-15)** — la nota de ROLLBACK F6 de arriba ("no reintroducir") queda superada: se volvió a implementar (`NivelCuentaSubio.tsx`, spec en `docs/audits/LEVEL-UP-ANIMACION-2026-09-08.md`, `0156_nivel_cuenta_subio_expuesto.sql`) con la causa real del incidente anterior ya identificada — según `docs/FINAL-SPRINT-REPORT.md` esa vez el problema fue que la migración `0120` (que redefine `insertar_intento`/`insertar_intento_logica`) no estaba aplicada en la base en ese momento, no un bug de la lógica de nivel de cuenta en sí. Wireado en los 13 resúmenes (mismo criterio que `NivelMundoSubio`), verificado tsc/eslint/vitest 153/153 limpio.

- **Link de confirmación roto, recurrencia (2026-09-15)** — mismo síntoma y misma causa raíz que la entrada de arriba ("link de confirmación sin https://"), reapareció en el dominio real de producción (`prodigia-sandy.vercel.app`). Diagnosticado de nuevo con el código como referencia (nada tocado en `RegistroForm.tsx`/`RecuperarForm.tsx`/`ConvertirCuenta.tsx`/`urlAbsoluta.ts` desde la vez anterior, siguen armando la URL bien) — la causa fue, otra vez, la configuración del dashboard de Supabase. **Confirmado arreglado por el usuario**: Site URL = `https://prodigia-sandy.vercel.app`, Redirect URLs = `https://prodigia-sandy.vercel.app/**`. Login/registro/recuperar funcionando de nuevo. Nota para la próxima vez que se repita: revisar primero si el dominio de producción cambió (dominio propio nuevo, redeploy de Vercel) antes de sospechar del código — este bug nunca fue del código en ninguna de las dos veces.

- **Link de confirmación roto — TERCERA causa distinta, ahora sí de arquitectura (2026-09-15)** — después de arreglar Site URL/Redirect URLs (entrada de arriba) y de que el correo ya llegaba bien (SMTP de Brevo funcionando), el link seguía sin confirmar. Causa real: `/auth/callback` usa el flujo **PKCE** (`exchangeCodeForSession(code)`), que necesita una cookie `code_verifier` que Supabase guarda en el navegador que llamó `signUp()`/`resend()`. Si el link del correo se abre en OTRO navegador o contexto (el celular en vez de la compu donde se registró, o la app de Gmail que abre los links en su propio WebView) esa cookie no existe ahí y la confirmación falla SIEMPRE — no es intermitente, es una limitación estructural del PKCE para links que viajan por email, no un bug de configuración.
  **Arreglado en código**: nueva ruta `src/app/auth/confirm/route.ts`, que usa `supabase.auth.verifyOtp({ type, token_hash })` en vez de `exchangeCodeForSession` — no depende de ninguna cookie del navegador que originó el pedido, así que confirma sin importar dónde se abra el link. Es el método que la propia documentación de Supabase recomienda para links de email.
  **Falta un paso en el dashboard que el código NO puede hacer solo**: las plantillas de email (Authentication → Email Templates) siguen mandando `{{ .ConfirmationURL }}`, que apunta al viejo endpoint `/auth/v1/verify` (flujo PKCE). Hay que editar cada plantilla y reemplazar el link por uno a `/auth/confirm` con `token_hash`:
  ...
  - **Reset Password**: `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=recovery&next=${encodeURIComponent("/auth/actualizar-password")}` → en la práctica: `.../auth/confirm?token_hash={{ .TokenHash }}&type=recovery&next=%2Fauth%2Factualizar-password`
  La ruta vieja (`/auth/callback?code=...`) se deja intacta (no rompe nada dejarla) por si algún flujo la sigue usando, pero deja de ser el camino real una vez cambiadas las plantillas.
  **Aparte, sin resolver todavía**: el usuario reportó que tras el primer intento fallido, volver a registrarse con el mismo email no manda un correo nuevo, y el botón "reenviar" tampoco — candidatos: (a) rate-limit propio de Supabase para emails de auth (Authentication → Rate Limits, separado del límite de Brevo), o (b) Brevo reteniendo el envío por revisión de remitente nuevo (mismo tema pendiente de la entrada de SMTP más arriba). `mensajeErrorAuth` ya loguea `error.code` en la consola del navegador para el próximo intento — pedirle al usuario que mire la consola (F12) la próxima vez que el reenvío "no haga nada" para saber cuál de las dos es.

- **Fase D (2026-09-17) — desglose por mundo/sub-tema del grupo, gateado Pro**: el dashboard del profesor (`/profesor/[groupId]`) ganó un desglose de progreso que no existía antes, con el mismo criterio de "estadísticas avanzadas" individual (0146/0149/0151, `/perfil/estadisticas`) llevado a nivel de grupo:
  - **Gratis, para cualquier profesor**: sección nueva "Nivel por mundo" en `[groupId]/page.tsx` — nivel 1-100 (`world_progress`) por alumno y por uno de los 10 mundos (`MUNDOS_LANDING`), mismo patrón visual de dot + opacidad que ya tenía la tabla de nivel 1-10 por operación aritmética. RPC nueva `resumen_grupo_mundos(p_group_id)`.
